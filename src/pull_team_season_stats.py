...
def main() -> None:
    ...
    for year in range(start, end + 1):
        print(f'Pulling team season stats for {year}...')
        try:
            df = client.get_df('/stats/season', {'year': year, 'seasonType': season_type})
        except requests.HTTPError as e:
            print(f'WARNING: team season stats pull failed for {year}: {e}')
            continue
        if not df.empty:
            df['season'] = year
            frames.append(df)
    ...
    out = write_df(stats, 'data/raw/cfbd_team_season_stats.csv')
