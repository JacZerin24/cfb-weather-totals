...
def _section_html(general_count: int, fcs_count: int, has_card: bool) -> str:
    ...
    <section id="bankroll">
      <div class="section-head">
        ...
        <p>Turns the live board into a simple staking checklist without changing the model.</p>
      </div>
      <div class="bankroll-grid">
        <div class="bankroll-panel">
          <div class="bankroll-controls">
            ...
            <label><span>Risk profile</span>
              <select id="bankrollProfile">
                ...
                <option value="standard">Historical-method standard</option>
                <option value="custom">Custom</option>
              </select>
            </label>
            <label><span>Straight risk %</span><input id="straightRiskPct" type="number" min="0" max="5" step="0.05" value="0.50"></label>
            <label><span>2-leg card risk %</span><input id="cardRiskPct" type="number" min="0" max="5" step="0.05" value="0.25"></label>
          </div>
          <div class="bankroll-metrics">
            <div><span>Straight stake</span><strong id="straightStake">—</strong></div>
            <div><span>2-leg card stake</span><strong id="cardStake">—</strong></div>
          ...
          <div class="bankroll-note">The default profile is intentionally conservative while 2026 is being prospectively tracked. Amounts are flat-risk suggestions, not Kelly sizing. Your bankroll is stored only in this browser.</div>
        </div>
        <div class="bankroll-panel guide-panel">
          <div class="eyebrow">What the historical method actually means</div>
          <h3>What should I be in?</h3>
          <ol>
            <li><strong>General-track QUALIFIES:</strong> if following the historical straight-bet method, take each one once as a straight UNDER. Do not cherry-pick only the ones that look best after they qualify.</li>
            <li><strong>2-leg card:</strong> add only the single top card shown by the site when two eligible general qualifiers exist. Do not create extra 2-leg combinations from the same slate.</li>
            <li><strong>FCS QUALIFIES:</strong> keep these paper-only by default. The FCS screen is exploratory and is intentionally excluded from the validated legacy card.</li>
            <li><strong>LEAN / WATCH / NO PLAY:</strong> no bankroll stake under the current method.</li>
            <li><strong>Site refreshes:</strong> the same game appearing again Friday or Saturday is not a new wager. Stake it once, unless you deliberately adopt a separately tested re-entry rule later.</li>
          </ol>
          <p class="small">Current snapshot: <strong>{general_count}</strong> general qualifier(s), <strong>{fcs_count}</strong> FCS qualifier(s), and {escape(card_text)}.</p>
        </div>
      </div>
      <div class="bankroll-panel plan-panel">
        <div class="plan-head"><div><div class="eyebrow">Current-board translation</div><h3>Suggested staking checklist</h3></div><span class="small">The card overlaps two straight-bet exposures, so its default stake is smaller.</span></div>
...
BANKROLL_CSS = r'''
    ...
    .bankroll-controls { display:grid; grid-template-columns:1.15fr 1.35fr .8fr .8fr; gap:10px; align-items:end; }
    ...
    .bankroll-metrics { display:grid; grid-template-columns:repeat(4,1fr); gap:9px; margin-top:14px; }
    ...
    .stake-paper { color:#ddd6fe; border:1px solid rgba(139,92,246,.35); background:rgba(139,92,246,.12); }
    .stake-empty { color:var(--muted); border:1px dashed var(--line); border-radius:13px; padding:15px; }
    @media(max-width:1000px) { .bankroll-grid { grid-template-columns:1fr; } .bankroll-controls { grid-template-columns:repeat(2,1fr); } }
...
def _script(general: list[dict[str, Any]], fcs: list[dict[str, Any]], card: list[dict[str, Any]]) -> str:
  ...
  <script>
    (function() {{
      ...
      const profiles = {{
        conservative: {{ straight:0.50, card:0.25 }},
        standard: {{ straight:1.00, card:0.50 }}
      }};
      ...
      function render() {{
        ...
        const totalRisk = straightStake * bankrollData.general.length + (hasCard ? cardStake : 0);
        ...
        if (bankroll && exposure > 7.5) {{
          warningEl.textContent = 'This snapshot would put ' + exposure.toFixed(1) + '% of the bankroll at risk. Consider the conservative profile or a smaller custom percentage rather than increasing exposure just because the slate is busy.';
          warningEl.hidden = false;
        }} else if (straightPct > 1.5 || cardPct > 0.75) {{
          warningEl.textContent = 'These custom percentages are above the site’s normal planning range. Historical returns are not certain enough to justify aggressive sizing by default.';
          warningEl.hidden = false;
        }}

        const rows = [];
        bankrollData.general.forEach(function(game) {{
          rows.push('<tr><td><span class="stake-kind stake-straight">STRAIGHT</span></td><td><strong>' + esc(game.matchup) + '</strong><br><span class="small">UNDER ' + esc(game.total) + ' · edge ' + esc(game.edge) + '</span></td><td>' + (bankroll ? '<strong>' + money(straightStake) + '</strong>' : 'Enter bankroll') + '</td><td class="small">One flat stake. Do not scale up solely because the edge number is larger.</td></tr>');
        }});

        if (hasCard) {{
          const a = bankrollData.card[0];
          const b = bankrollData.card[1];
          rows.push('<tr><td><span class="stake-kind stake-card">2-LEG CARD</span></td><td><strong>UNDER ' + esc(a.total) + ' ' + esc(a.matchup) + '</strong><br><strong>UNDER ' + esc(b.total) + ' ' + esc(b.matchup) + '</strong></td><td>' + (bankroll ? '<strong>' + money(cardStake) + '</strong>' : 'Enter bankroll') + '</td><td class="small">The one site-selected card only. It overlaps two straight positions, so the stake is smaller.</td></tr>');
        }}

        bankrollData.fcs.forEach(function(game) {{
          rows.push('<tr><td><span class="stake-kind stake-paper">PAPER ONLY</span></td><td><strong>' + esc(game.matchup) + '</strong><br><span class="small">UNDER ' + esc(game.total) + ' · FCS edge ' + esc(game.edge) + '</span></td><td><strong>$0 suggested</strong></td><td class="small">Track prospectively; FCS has not earned the same bankroll treatment as the general strategy.</td></tr>');
        }});

        if (!rows.length) {{
          planEl.innerHTML = '<div class="stake-empty"><strong>No staking actions from this snapshot.</strong><br>There are no qualifying general straight bets, no valid two-leg card, and no FCS paper qualifier right now.</div>';
        }} else {{
...
def main() -> None:
    ...
    print(f'Added bankroll helper: {len(general)} general qualifier(s), {len(fcs)} FCS paper qualifier(s), card={len(card) == 2}.')
