...
def orientation_table() -> pd.DataFrame:
    ...
    keep = [c for c in ['venue_id', 'field_axis_deg', 'axis_uncertainty_deg', 'roof_behavior', 'verification_status'] if c in orientation.columns]
...
def live_venue_map(season: int) -> pd.DataFrame:
    client = CFBDClient()
    games = pd.json_normalize(client.get('/games', {'year': season, 'seasonType': 'regular'}))
    if games.empty:
        return pd.DataFrame(columns=['game_id', 'venue_id'])
    games = games.rename(columns={'id': 'game_id', 'venueId': 'venue_id'})
    games['game_id'] = pd.to_numeric(games['game_id'], errors='coerce')
    games['venue_id'] = pd.to_numeric(games['venue_id'], errors='coerce')
    return games[['game_id', 'venue_id']].drop_duplicates('game_id')
...
def challenger_status(prediction: float, total: float, weather_ready: bool) -> str:
...
def score_challenger(board: pd.DataFrame) -> pd.DataFrame:
    ...
    live['challenger_status'] = [
        challenger_status(
            ...
            str(status).lower() in {'ok', 'indoor'},
        )
        for pred, total, status in zip(live['challenger_pred_market_residual'], live['closing_total'], live['nws_status'])
    ]
    return live
...
def main() -> None:
    ...
    board = board.merge(live_venue_map(season), on='game_id', how='left')
