...
PROTOCOL_PATH = ROOT / "config/joint_core_evaluation_protocol_2026.yml"
CHALLENGER_PATH = ROOT / "config/joint_core_challenger_2026.yml"
...
def load_protocol() -> dict[str, Any]:
    ...
    if int(protocol.get("season", 0)) != SEASON:
        raise RuntimeError("Joint-core evaluation protocol must remain scoped to 2026.")
    return protocol
...
def challenger_sha256() -> str:
    return hashlib.sha256(CHALLENGER_PATH.read_bytes()).hexdigest()
...
def validate_protocol() -> None:
    ...
    drift = [
        f"{name}: joint_core={expected!r}, production={actual!r}"
        for name, expected, actual in checks
        if expected != actual
    ]
    if drift:
        raise RuntimeError(
            "Joint-core shadow entry timing drifted from the official prospective protocol: "
            + "; ".join(drift)
        )

    challenger = load_yaml("config/joint_core_challenger_2026.yml")
    if str(challenger.get("version")) != str(protocol["challenger_version"]):
        raise RuntimeError("Frozen joint-core challenger version no longer matches evaluation protocol.")
    if str(challenger.get("status")) != "research_only_shadow":
        raise RuntimeError("Joint-core challenger is no longer marked research-only shadow.")
    if pd.Timestamp(challenger["frozen_at_utc"]) != pd.Timestamp(protocol["frozen_at_utc"]):
        raise RuntimeError("Joint-core challenger and evaluation protocol freeze times differ.")
    if str(challenger["challenger"]["training_cutoff"]).strip() != "season < 2026":
        raise RuntimeError("Joint-core challenger training cutoff is no longer frozen before 2026.")
...
def select_official_shadow_entries(snapshots: pd.DataFrame) -> pd.DataFrame:
    ...
    work["entry_lead_minutes"] = (
        work["start_date"] - work["captured_at_utc"]
    ...
    ready = work.get("joint_core_ready", pd.Series(False, index=work.index)).map(as_bool)
    fbs_vs_fbs = work.get("fbs_vs_fbs", pd.Series(False, index=work.index)).map(as_bool)
    outdoor = work.get("outdoor", pd.Series(False, index=work.index)).map(as_bool)
    ...
    selected = work[
        ...
        & fbs_vs_fbs
        & outdoor
        & work.get("division_track", pd.Series("", index=work.index)).astype(str).str.upper().eq(
            str(protocol["scope"]["division_track"]).upper()
        )
        & work.get("challenger_version", pd.Series("", index=work.index)).astype(str).eq(
            str(protocol["challenger_version"])
        )
        & work.get("evaluation_protocol_version", pd.Series("", index=work.index)).astype(str).eq(
    ...
    selected = selected.sort_values(
        ["github_run_id", "game_id", "github_run_attempt", "captured_at_utc"]
    )
    ...
    selected["official_joint_core_entry"] = True
...
def final_results(client: CFBDClient | None = None) -> pd.DataFrame:
    ...
    return games[
        ["game_id", "final_home_points", "final_away_points", "actual_total_points"]
...
def grade_entries(entries: pd.DataFrame, client: CFBDClient | None = None) -> pd.DataFrame:
    ...
    out["result_vs_entry_total"] = [
        result_vs_total(a, t) for a, t in zip(out["actual_total_points"], out["entry_total"])
    ]

    strength = {
        str(k): int(v) for k, v in protocol["classification_evaluation"]["strength_order"].items()
    }
...
def qualifier_summary(graded: pd.DataFrame, prefix: str) -> dict[str, Any]:
    ...
    return {
        ...
        f"{prefix}_avg_clv_points": float(
            pd.to_numeric(q[f"{prefix}_qualifier_clv_points"], errors="coerce").mean()
        ),
    }


def prospective_signal(n: int, delta: float, ci_high: float) -> str:
    if n < 5:
        return "INSUFFICIENT_SAMPLE"
    if np.isfinite(delta) and delta < 0 and np.isfinite(ci_high) and ci_high < 0:
        return "PROSPECTIVE_MAE_SUPPORTED"
    if np.isfinite(delta) and delta < 0:
        return "PROSPECTIVE_MAE_FAVORABLE_UNCERTAIN"
    return "PROSPECTIVE_MAE_NOT_FAVORABLE"
...
def build_summary(graded: pd.DataFrame) -> pd.DataFrame:
    ...
    if graded.empty:
        row = {
            ...
            "challenger_config_sha256": challenger_sha256(),
            "challenger_version": protocol["challenger_version"],
            "graded_paired_games": 0,
            "prospective_signal": "INSUFFICIENT_SAMPLE",
        }
    ...
    ci_low, ci_high = bootstrap_mae_delta(
        graded["challenger_minus_baseline_abs_error"].to_numpy(float), protocol
    )
    migrations = graded[
        graded["status_migration"].ne(0) & graded["migration_correct"].notna()
    ].copy()
    row = {
        "protocol_version": protocol["protocol_version"],
        "protocol_sha256": protocol_sha256(),
        ...
        "through_week": int(pd.to_numeric(graded["week"], errors="coerce").max())
        if graded["week"].notna().any()
        else np.nan,
        ...
        "prospective_signal": prospective_signal(len(graded), delta, ci_high),
        ...
        "correct_status_migrations": int(migrations["migration_correct"].astype(bool).sum())
        if len(migrations)
        else 0,
        "status_migration_accuracy": float(migrations["migration_correct"].astype(float).mean())
        if len(migrations)
        else np.nan,
        "mean_crosswind_mph": float(graded["crosswind_mph"].mean()),
        "mean_alongwind_mph": float(graded["alongwind_mph"].mean()),
        "mean_temperature_anomaly_f": float(graded["temperature_anomaly_f"].mean()),
        "mean_local_wind_percentile": float(graded["wind_local_percentile"].mean()),
    }
    row.update(qualifier_summary(graded, "baseline"))
    row.update(qualifier_summary(graded, "challenger"))
    return pd.DataFrame([row])
...
def write_outputs(entries: pd.DataFrame, graded: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    disagreements = (
        graded[
            graded.get("status_disagreement", pd.Series(False, index=graded.index)).fillna(False)
        ].copy()
        if not graded.empty
        else pd.DataFrame()
    )
    ...
    lines = [
        ...
        f"Prospective signal: **{s.get('prospective_signal', 'INSUFFICIENT_SAMPLE')}**  ",
        f"Graded paired joint-core-ready games: **{n}**",
        "",
    ]
    if n:
        lines += [
        ...
        lines += ["No completed official post-freeze joint-core shadow entries are available yet.", ""]
    lines += [
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Frozen 2026 baseline-vs-joint-core prospective evaluator."
    )
    parser.add_argument(
        "command", nargs="?", choices=["validate-protocol", "build"], default="build"
    )
    args = parser.parse_args()
    if args.command == "validate-protocol":
        validate_protocol()
        print(f"Joint-core evaluation protocol valid: {protocol_sha256()}")
    else:
        build()
